...
function openAudioDB() {
    return new Promise((resolve, reject) => {
        ...
        const request = indexedDB.open('golden-record-audio', 3);
...
async function loadSelectedAudio() {
    ...
    return new Promise(resolve => {
        try {
            ...
            const currentReq = tx.objectStore('meta').get('current');
            currentReq.onsuccess = () => {
                const id = currentReq.result;
                if (!id) { db.close(); return resolve(null); }
                const fileReq = tx.objectStore('files').get(id);
                fileReq.onsuccess = () => {
                    const value = fileReq.result;
                    db.close();
                    if (!value) return resolve(null);
                    const buffer = value.buffer instanceof ArrayBuffer ? value.buffer :
                        (value.buffer?.buffer instanceof ArrayBuffer ? value.buffer.buffer : null);
                    if (!buffer) return resolve(null);
                    resolve({ ...value, buffer });
                };
                fileReq.onerror = () => { db.close(); resolve(null); };
            };
            currentReq.onerror = () => { db.close(); resolve(null); };
        } catch (_) { db.close(); resolve(null); }
    });
}

function setupVisiblePlayer(selected) {
    const audio = document.querySelector('#audioPlayer');
    const play = document.querySelector('#audioPlay');
    const stop = document.querySelector('#audioStop');
    const mute = document.querySelector('#audioMute');
    ...
    let resumeAfterDecoderLoad = false;
    ...
    if (buffer) {
        objectUrl = URL.createObjectURL(new Blob([buffer], { type: selected.type || 'audio/mpeg' }));
        audio.src = objectUrl;
        label.textContent = `[ ${selected.name.toUpperCase()} ]`;
    } else {
        audio.src = './src/assets/audio/voyager.mp3';
        label.textContent = '[ DEFAULT VOYAGER ]';
    }
    audio.volume = 1;

    function getImageIndexForTime(seconds) {
        const count = IMG_DATA.left.timeStamps.length;
        if (!count || !Number.isFinite(seconds)) return 0;

        // Keep the original scan-line timing for the Voyager track, but scale it
        // to the actual audio duration so uploaded WAV/MP3 files stay synchronized.
        const encodedDuration = IMG_DATA.left.timeStamps[count - 1] / SAMPLE_RATE;
        const ratio = audio.duration > 0 && encodedDuration > 0
            ? Math.min(1, seconds / audio.duration) * encodedDuration
            : seconds;
        const samplePosition = Math.max(0, ratio * SAMPLE_RATE);

        // Binary search: faster than walking all timestamps every animation frame.
        let lo = 0;
        let hi = count - 1;
        while (lo < hi) {
            const mid = Math.ceil((lo + hi) / 2);
            if (IMG_DATA.left.timeStamps[mid] <= samplePosition) lo = mid;
            else hi = mid - 1;
        }
        return Math.max(0, Math.min(count - 1, lo));
    }

    function syncImageToAudio(force = false) {
        if (!audio.duration || !IMG_DATA.left.timeStamps.length) return;
        const index = getImageIndexForTime(audio.currentTime);
        if (force || index !== lastSyncedOffset) {
            lastSyncedOffset = index;
            IMG_DATA.changeIndex = true;
            IMG_DATA.offset = index;
            if (dom.imgSelector) dom.imgSelector.value = `${index}`;
            IMG_DATA.left.go = true;
            IMG_DATA.right.go = true;
        }
    }

    function runAudioImageSync() {
        syncImageToAudio(false);
        if (progress && audio.duration) progress.value = (audio.currentTime / audio.duration) * 100;
        if (!audio.paused && !audio.ended) {
            syncAnimationId = requestAnimationFrame(runAudioImageSync);
        } else {
            syncAnimationId = null;
        }
    }

    function startAudioImageSync() {
        if (syncAnimationId === null) syncAnimationId = requestAnimationFrame(runAudioImageSync);
    }

    function stopAudioImageSync() {
        if (syncAnimationId !== null) cancelAnimationFrame(syncAnimationId);
        syncAnimationId = null;
    }
    ...
    play?.addEventListener('click', async () => {
        try {
            ...
            status.textContent = '[ ERROR ] Browser blocked playback. Click PLAY again.';
        }
    });
    ...
    audio.addEventListener('loadedmetadata', () => {
        ...
        syncImageToAudio(true);
        if (resumeAfterDecoderLoad) {
            resumeAfterDecoderLoad = false;
            audio.play().catch(() => {});
        }
    });
    ...
    audio.addEventListener('play', () => {
        IMG_DATA.pause = false;
        startAudioImageSync();
        play.textContent = '❚❚ PLAYING';
        play.classList.add('playing');
        status.textContent = '[ PLAYING — IMAGE/AUDIO SYNC ]';
        syncImageToAudio(true);
    });
    ...
    audio.addEventListener('pause', () => {
        ...
        status.textContent = '[ PAUSED — IMAGE/AUDIO SYNC ]';
    });
    ...
    audio.addEventListener('error', () => {
        status.textContent = '[ ERROR ] Audio could not be played.';
    });

    updateMuteButton();
    window.__goldenRecordAudio = audio;
    window.__goldenRecordObjectUrl = objectUrl;
}
 ...
 */
function getAudio() {
    const loadDefaultAudio = () => fetch('./src/assets/audio/voyager.mp3').then(response => {
        if (!response.ok) throw new Error('Default audio could not be loaded.');
        return response.arrayBuffer();
    });
    loadSelectedAudio().then(selected => {
        setupVisiblePlayer(selected);
        return selected?.buffer || loadDefaultAudio();
      ...
      .catch(error => {
        dom.loader.querySelector('.loader-sub').textContent='[ ERROR ] Could not decode audio. Check the uploaded file.';
