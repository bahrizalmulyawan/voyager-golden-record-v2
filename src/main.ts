...
function getSyncedImageIndex(currentTime: number, duration: number): number {
    const count = IMG_DATA.left.timeStamps.length;
    if (!count || !Number.isFinite(currentTime)) return 0;

    const encodedDuration = IMG_DATA.left.timeStamps[count - 1] / SAMPLE_RATE;
    const ratio = duration > 0 && encodedDuration > 0
        ? Math.min(1, currentTime / duration) * encodedDuration
        : currentTime;
...
function getAudio(): void {
    const VOYAGER_AUDIO_PATHS = ['./assets/audio/voyager.mp3', './assets/audio/voyager.mp3', './public/assets/audio/voyager.mp3'];
    const voyagerPath = VOYAGER_AUDIO_PATHS[0];
    const loadVoyagerAudio = async () => {
        let lastError = null;
        for (const path of VOYAGER_AUDIO_PATHS) {
            try {
        ...
        throw lastError || new Error('Voyager MP3 could not be loaded.');
    };

    // LIVE DEMO is intentionally disabled: always use the bundled Voyager MP3.
    setupVisiblePlayer(null);
    loadVoyagerAudio()
        ...
        .catch(error => {
            console.log('Error fetching or decoding bundled Voyager audio: ', error);
            if (dom.loader) dom.loader.innerHTML = '<p>[ ERROR ] AUDIO DECODER FAILED</p><p class="loader-sub">Could not load assets/audio/voyager.mp3.</p>';
        });
}
